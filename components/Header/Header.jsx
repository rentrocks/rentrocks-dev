...
import { ArrowPathIcon, ArrowRightOnRectangleIcon, Bars3Icon, BellAlertIcon, ChartPieIcon, ChatBubbleLeftEllipsisIcon, CursorArrowRaysIcon, FingerPrintIcon, ChatBubbleOvalLeftIcon, SquaresPlusIcon, UserIcon, XMarkIcon, HomeIcon, QuestionMarkCircleIcon, ShoppingCartIcon, TagIcon, HeartIcon } from '@heroicons/react/24/outline'
...
import { useCartSidebar } from '@/contexts/utils/cartSidebar'
import CartCard from '@/app/cart/CartCard'
import FavouriteCard from '@/app/favourite/FavouriteCard'
...
export default function Header() {
  const { isOpen, setIsOpen: setCartIsOpen } = useCartSidebar()
    ...
    <>
      ...
      <Searchbar />
          ...
          <div className='hidden lg:flex lg:flex-1 lg:justify-end space-x-8'>
            ...
            {currentUser && user && <div>
              <button onClick={() => setCartIsOpen(true)} className="flex gap-2 items-center text-sm font-bold leading-6 text-gray-900">
            ...
            {currentUser && user && <div>
              <button onClick={() => setOpen(true)} className="flex gap-2 items-center text-sm font-bold leading-6 text-gray-900">
                <span><HeartIcon className='h-5 w-5 font-bold' /> </span>  Fav <span className="font-bold text-white rounded-full bg-[#5842be] flex items-center justify-center font-mono h-5 w-5">{user.favoritesProducts.length}</span>
              </button>
            </div>}
                    ...
                    <div className="flex h-full flex-col overflow-y-scroll bg-white shadow-xl">
                      <div className="flex-1 overflow-y-auto px-4 py-6 sm:px-6">
                        <div className="flex items-start justify-between">
                          <Dialog.Title className="text-lg font-medium text-gray-900">Fvaourites</Dialog.Title>
                        ...
                        <div className="mt-8">
                          <div className="flow-root">
                            {user?.favoritesProducts?.map((productId) => {
                              return <FavouriteCard productId={productId} />
                            }
                            )}
                          </div>
                        </div>
